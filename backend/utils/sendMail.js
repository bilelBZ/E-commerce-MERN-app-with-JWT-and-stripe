...
const sendMail = async (id, email, option) => {
  ...
  if (option === "email verification") {
    ...
    const mailOptions = {
      from: "Omrani1996yasser@icloud.com", // sender address
      to: email,
      subject: "Confirm your email for Shopyas", // Subject line
      html: `<div>
  ...
  // send a mail for resetting password if forgot password
    ...
    const mailOptions = {
      from: "Omrani1996yasser@icloud.com", // sender address
      to: email,
      subject: "Reset Password for Shopyas", // Subject line
      html: `<div>
					<h2>Reset Password for your Shopyas account</h2>
					<br/>
